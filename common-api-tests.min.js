...
function checkTime(a){if("Number"===getType(a))if(0<a)pm.test("Response Time < "+convertTime(a),function(){pm.expect(pm.response.responseTime).to.be.below(a)});else throw new RangeError('Parameter value must be a strictly positive number for function "checkTime(time)"');else throw new TypeError('Parameter value must be a number for function "checkTime(time)"');}
function checkStatusCode(a){if("Number"===getType(a))switch(!0){case 100<=a&&199>=a:pm.test("Status Code (Information)",function(){pm.response.to.have.status(a)});break;case 200<=a&&299>=a:pm.test("Status Code (Success)",function(){pm.response.to.have.status(a)});break;case 300<=a&&399>=a:pm.test("Status Code (Redirection)",function(){pm.response.to.have.status(a)});break;case 400<=a&&499>=a:pm.test("Status Code (Client Error)",function(){pm.response.to.have.status(a)});break;case 500<=a&&599>=a:pm.test("Status Code (Server Error)",
function(){pm.response.to.have.status(a)});break;default:throw new RangeError('Parameter value must be an existing status code number for function "checkStatusCode(statusCode)"');}else throw new TypeError('Parameter value must be a number for function "checkStatusCode(statusCode)"');}
function checkContentType(a){if("String"===getType(a))pm.test("Content Type",function(){pm.expect(pm.response.headers.get("content-type")).to.include(a)});else throw new TypeError('Parameter value must be a string for function "checkContentType(contentType)"');}
function checkJSONSchema(a){if("Object"===getType(a)){var b=tv4.validate(pm.response.json(),a);pm.test(b?"JSON Schema":"JSON Schema ("+tv4.error.message+" for data path "+(tv4.error.dataPath?tv4.error.dataPath:"/")+")",function(){return pm.expect(b).to.be.true})}else throw new TypeError('Parameter value must be an object for function "checkJSONSchema(jsonSchema)"');}
function checkLocation(a){if("String"===getType(a))pm.test("Location",function(){pm.response.to.be.header("location",a)});else throw new TypeError('Parameter value must be a string for function "checkLocation(location)"');}function getRegexGUID(){return"^(?!00000000-0000-0000-0000-000000000000)([0-9a-zA-Z]{8}-[0-9a-zA-Z]{4}-[0-9a-zA-Z]{4}-[0-9a-zA-Z]{4}-[0-9a-zA-Z]{12})$"}
